...
mysql -u taotao -p123456 < setup_items_db.sql
if [ $? -ne 0 ]; then
...
print_color "yellow" "Starting product management backend service..."
python3 backend.py &
if [ $? -ne 0 ]; then
    print_color "red" "Failed to start backend service"
    exit 1
fi
print_color "green" "Backend service started in background, port 5003"

print_color "green" "======================="
print_color "green" "Product Management System Setup Complete!"
print_color "green" "API URL: http://localhost:5003/api/admin/items"
