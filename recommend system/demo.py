...
def save_to_db(rec_dict, algo='mix'):
    ...
    with conn.cursor() as cur:
        sql = """
        REPLACE INTO recommend_home(userKey, productID, rankNo, algoTag, score, createdAt)
        VALUES (%s,%s,%s,%s,%s,%s)
        """
...
def build_item_vectors():
    if os.path.exists(VEC_NPY): return
    ...
    txt_full=model.encode(text,batch_size=32,show_progress_bar=True,
    ...
    if d: emb=PCA(n_components=d,random_state=42).fit_transform(emb)
...
def get_baseline_candidates(vec,meta,hist,id2idx):
    ...
    for uid,seq in hist.items():
        ...
        cands=[int(i) for i in ind[0] if idx2pid[i] not in seq][:TOPK]
...
class SeqDataset(Dataset):
    ...
    def __getitem__(self,i):
        h,tgt=self.data[i]
        pad=MAXLEN-len(h)
        return (torch.tensor([0]*pad+h),torch.tensor(len(h)-1 if h else 0),
                torch.tensor(tgt))
...
def train_sasrec(vec,hist,id2idx):
    ...
    for ep in range(1,EPOCH+1):
        ...
        for s,l,t in dl:
            ...
            loss_=cel(m(s,l),t)
            opt.zero_grad();loss_.backward();opt.step()
            loss+=loss_.item()*len(t)
        print(f"Epoch {ep}/{EPOCH}  loss={loss/len(ds):.4f}")
    return m

def get_sasrec_candidates(model,vec,meta,hist,id2idx):
    if model is None: return {u:[] for u in hist}
    idx2pid=meta["productID"].tolist()
    model.eval()
    out={}
    for uid,seq in hist.items():
...
def mix_and_save(baseline, sasrec, meta, hist, id2idx,
    ...
    for uid, seq in hist.items():
        ...
        if len(seq) >= 2:
            tot += 1
            hit += int(seq[-1] in [idx2pid[i] for i in mix])

        # 5. 组织一行写 Excel
        row = [uid]
        rec_pid_list = []
        for idx in mix:
            pid = meta.iloc[idx]["productID"]
            rec_pid_list.append(pid)           # ← 收集给 DB 用
            row.extend([
                pid,
                meta.iloc[idx]["productName"],
                meta.iloc[idx]["brand"],
                meta.iloc[idx]["price"]
            ])
    ...
    # ---------- 打印指标 ----------
    print(f"🎯 Mix Hit@{topk}: {hit / tot:.4f}" if tot else "⚠️ 无法评估")
...
if __name__=="__main__":
    ...
    print("🔸 加载购物车日志...")
    df = fetch_sql("""
        SELECT IFNULL(CAST(userID AS CHAR), sessionID) AS uid,
               productID,
               createdAt
        FROM carts
        ORDER BY createdAt
